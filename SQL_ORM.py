...
def testing():
    player_id_generator = ID()
    team_id_generator = ID()
    pac = Division("Pacific")
    lakers = Team(team_id_generator.get_next(), "Lakers", "California", "Los Angeles", pac, "Staples Center", 16, "www.nba.com/lakers")
    kobe = Player(player_id_generator.get_next(), "Kobe", "Bryant", 24, 39, 5, "American", lakers.id)
    me = Player(player_id_generator.get_next(), "Omer", "Dekel", 17, 17, 0, "Israeli", lakers.id)
    my_orm = ORM()
    my_orm.team.add_team(lakers)
    my_orm.player.add_player(kobe)
    my_orm.player.add_player(me)
    print("FJFJFF")
    print(my_orm.team.get_teams())
    for team in my_orm.team.get_teams("<", state=lakers.state):
        print("!!!!!!")
        print(team)
        print(team.id)
    for team in my_orm.team.get_teams_contains(state="Cal"):
        print("!!!!!!!!!!")
        print(team)
        print(team.id)
    for player in my_orm.player.get_players(first_name=me.first_name, last_name=me.last_name):
        print("!!!!!!!!!!!!!!")
        print(player)
        print(player.id)
    for player in my_orm.player.get_players_contains(first_name="Omer", last_name="eke"):
        print("!!!!!!!!!!!!!!!!!!")
        print(player)
        print(player.id)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("PLAYER ", me.first_name, me.number, me.age)
    print("PLAYER ", my_orm.player.update_player(me, first_name="LALALA", number=56, age=84))
    me = my_orm.player.get_players(id=me.id)[0]
    print("PLAYER ", me.first_name, me.number, me.age)
    print("TEAM ", lakers.name)
    print("TEAM ", my_orm.team.update_team(lakers, name="Los Angeles Lakers"))
    lakers = my_orm.team.get_teams(id=lakers.id)[0]
    print("TEAM ", lakers.name)
    print("HELLO")
